...
namespace LBG.Editor.Drawers
{
	...
	internal sealed class SubclassSelectorFieldAttributeDrawer<T> : OdinAttributeDrawer<SubclassSelectorAttribute, T> where T : class
	{
		...
		private void DrawDropdown(GUIContent label)
		{
			var attr = this.Attribute;
			var property = this.Property;
			bool drawChildren = !attr.HideChildProperties;
			...
			{
				if (attr.DrawClassFoldout && drawChildren)
				{
					...
					{
						DrawChildren(property, drawChildren);
					}
				...
				// local method
				static void DrawChildren(InspectorProperty property, bool drawChildren)
				{
					// TODO: Feels weird. Maybe rethink.
